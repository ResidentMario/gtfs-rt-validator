...
pub enum ValidationError {
    IOError(io::Error),
    InvalidDataError(io::Error),
}

impl fmt::Display for ValidationError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        // Because `&self` is declared in the function signature, it is a pointer. As a result,
        // `match self` would require match arms prefixed with `&`: `&ValidationError::IOError`
        // etcetera.
        //
        ...
        // It uses `*self` and `ref ValidationError::[...]`
        ...
        match self {
            ...
            ValidationError::InvalidDataError(err) => {
                write!(f, "InvalidDataError: {}", err)
            }
        }
    }
}
...
// `ValidationError` every time we need it. E.g. otherwise every time we want to provide a `Result`
// we would have to type out the full `Result<T, ValidationError>` in our signature.
//
...
//
// Ok, next question, why Box<dyn Error> instead of the simpler ValidationError?
//
...
// into our result type. If we were to use `result::Result<T, ValidationError>` instead, any time
// internal code throws a different error, we'd have to convert that other error into a
// `ValidationError`; otherwise we wouldn't be able to use `Result<T, ValidationError>` as our return
...
// the boxing manually, but we'd still need to construct the ValidationError object, with e.g.:
// Err(ValidationError::NetworkError(io::Error::new(io::ErrorKind::Other, msg)))?
//
...
// `ValidationError::new(ValidationError::IOError(io::Error::new(...)))`  // ok
// `ValidationError::new(ValidationError::IOError)  // not ok
//
...
// `ValidatorError` for our purposes.
//
...
#[derive(Debug)]
pub enum ErrKind {
    IOError,          // Generic I/O error such as a nonexistant filename
    InvalidDataError, // The file cannot be read as a Protobuf
}

impl ValidationError {
    pub fn new(kind: ErrKind, msg: String) -> ValidationError {
        match kind {
            ErrKind::IOError => ValidationError::IOError(io::Error::new(io::ErrorKind::Other, msg)),
            ErrKind::InvalidDataError => {
                ValidationError::InvalidDataError(io::Error::new(io::ErrorKind::InvalidData, msg))
            }
        }
    }

    pub fn wrap(kind: ErrKind, err: impl Error, msg: String) -> ValidationError {
        ValidationError::new(kind, format!("{}: {}", msg, err))
    }
}
